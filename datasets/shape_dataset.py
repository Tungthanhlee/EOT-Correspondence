...
@DATASET_REGISTRY.register()
class PairPancreasDataset(Dataset):
    def __init__(self,
                 data_root, phase='train', return_faces=True,
                 return_evecs=True, num_evecs=200,
                 return_corr=False, return_dist=False):
        """
        Pair Pancreas Dataset
        ...
        if self.phase == 'train':
            self.off_files = self.off_files[:60]
            self.combinations = list(product(range(len(self.off_files)), repeat=2))
            self._size = len(self.combinations)
    ...
    def _init_data(self):
        # check the data path contains .off files
        off_path = os.path.join(self.data_root, 'pancreas')
        assert os.path.isdir(off_path), f'Invalid path {off_path} not containing .off files'
        off_files = sort_list(glob(f'{off_path}/*.ply'))
        off_files = [off for off in off_files if '0021' not in off]
        self.off_files = [off for off in off_files if '0061' not in off]
    ...
    def get_one_item(self, index):
        ...
        # get eigenfunctions/eigenvalues
        if self.return_evecs:
            item = get_spectral_ops(item, num_evecs=self.num_evecs, cache_dir=os.path.join(self.data_root, 'pancreas/diffusion'))
    ...
    def __getitem__(self, index):
        ...
        item = dict()
        item['first'] = self.get_one_item(first_index)
        item['second'] = self.get_one_item(second_index)
