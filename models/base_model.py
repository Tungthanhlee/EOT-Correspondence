...
class BaseModel:
    ...
    @torch.no_grad()
    def validation(self, dataloader, tb_logger, wandb=None, update=True):
        """Validation function.
        ...
        if len(geo_errors) != 0:
            ...
            if wandb is not None:
                wandb.log({'val auc': auc, 'val avg error': avg_geo_error})
            if tb_logger is not None:
                ...
                #save geo_errors
                np.save(os.path.join(self.opt['path']['visualization'], 'geo_errors.npy'), geo_errors)
